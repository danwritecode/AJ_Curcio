<template>
  <nav aria-label="Progress" class="hidden lg:block w-64">
    <ol role="list" class="space-y-6">
      <li v-for="step in steps" :key="step">
        <div v-if="step === currentStep && !completedSteps.includes(step) && !formSubmitted" class="flex items-start" aria-current="step">
  ...
  <nav class="lg:hidden flex items-center mb-3" aria-label="Progress">
    ...
    <ol role="list" class="ml-6 flex items-center space-x-5">
      <li v-for="step in steps" :key="step">
        <!-- Completed Step -->
        <span v-if="completedSteps.includes(step) || formSubmitted" class="block w-2.5 h-2.5 bg-red-600 rounded-full">
          <span class="sr-only">Step 1</span>
        </span>
        <!-- Current Step -->
        <span v-if="step === currentStep && !completedSteps.includes(step) && !formSubmitted" class="relative flex items-center justify-center" aria-current="step">
